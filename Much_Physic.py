...
class Particle:
    ...
    def update(self, particles):
        self.pos.update(self.pos + self.velocity)
        ...
        for particle in particles:
            if particle.id != self.id:
                ...
                opp_between = pg.Vector2(particle.pos.x - self.pos.x, particle.pos.y - self.pos.y)
                if opp_between.magnitude() != 0:
                    force = G*(particle.mass / (opp_between.magnitude() ** 2))
...
pg.display.set_caption("Much Physic Simulation")
...
particle_size = 20
size = smol_font.render("Particle Size", True, (255, 255, 255))
...
weight_slider_pos = ((particle_weight / 5) + 5, 113)
...
univ = smol_font.render("Universal Gravity", True, (255, 255, 255))
...
while running:
        ...
        if event.type == pg.QUIT:
            ...
            if paused:
                if event.key == pg.K_PERIOD:
                    text = "Simulation Advanced 1 Frame"
                    text_time_left = 30
                    for particle in particles:
    ...
    if mouse_down:
        if size_sliding:
            ...
            particle_size = (size_slider_pos[0] - 5) / 4
        elif weight_sliding:
            weight_slider_pos = (coerce_in(mouse_pos[0], (13, 205)), 113)
            particle_weight = (weight_slider_pos[0] - 5) * 5
        elif elas_sliding:
    ...
    if text_time_left > 0:
        text_time_left -= 1
        sim.blit(font.render(text, True, ((255 - 50) * (text_time_left / 30) + 50, (255 - 50) * (text_time_left / 30) + 50, (255 - 50) * (text_time_left / 30) + 50)), (10, res[1] - 35))
